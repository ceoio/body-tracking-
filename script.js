...
function processPlank(keypoints, minConfidence) {
    ...
    if (leftShoulder && rightShoulder && leftElbow && rightElbow && leftWrist && rightWrist &&
        ...
        if (!isInPlankPosition && isPlankInProgress) {
            ...
            if (!areElbowsProperlyBent) {
                if (leftElbowAngle < 65 || rightElbowAngle < 65) {
                    updateResult("Bend your elbows more");
                } else if (leftElbowAngle > 115 || rightElbowAngle > 115) {
                    updateResult("Don't bend your elbows too much");
                }
            } else if (!isBodyAligned) updateResult("Keep your body horizontal");
            else if (!areElbowsUnderShoulders) updateResult("Keep elbows under shoulders");
            else if (!isHipLevel) updateResult("Keep hips level with shoulders and ankles");
            else updateResult("Stay stable in plank position");
        ...
        isPlankInProgress = false;
        PLANK_POSITION_MEMORY.length = 0;
        updateResult("Position not detected clearly");
    }
}
...
// Update the skipQuestion function to handle the final submission
